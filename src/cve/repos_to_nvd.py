...
filter_cve_data = os.path.join(
    os.path.dirname(__file__), "queries/select_cve_vendor_product.sql"
)
    ...
    def resolve(self):
        matches_set = set(
            [
                ...
                # for a, b in filter(lambda match: match[0] != "n/a", self.matches)
...
def find_repo_matches(repos: list[Repo], cursor: psycopg2.extensions.cursor):
    ...
    with open(Path(filter_cve_data), "r") as f:
        ...
        for repo in tqdm(repos, desc="Finding Repo Matches"):
            ...
            url = repo.url
            cursor.execute(filter_cve_vendor_product, (f"{test_product}", f"{url}", f"%{url}%"))
